...
const SupportTicketsPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className={`min-h-screen ${resolvedTheme === 'dark' ? 'bg-transparent text-gray-200' : 'bg-white text-gray-900'}`}>
      ...
      {userRole && userRole.includes('support') && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
          <div className={`${resolvedTheme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-white border border-gray-200'} rounded-xl p-6`}>
            <h2 className={`text-2xl font-bold mb-4 ${resolvedTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              {t('support.dashboard.title')}
            </h2>
            <p className={`${resolvedTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-6`}>
              {t('support.dashboard.subtitle')}
            </p>
            <button
              onClick={() => navigate('/all-support-tickets')}
      ...
      <div className="bg-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="text-center">
            <h1 className={`text-4xl md:text-5xl font-bold mb-4 ${resolvedTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              {t('support.title')}
            </h1>
            <p className={`${resolvedTheme === 'dark' ? 'text-xl text-gray-300' : 'text-xl text-gray-600'} max-w-3xl mx-auto`}>
              {t('support.subtitle')}
            </p>
            ...
            <div className="flex justify-start mb-6">
              ...
              >
                ...
                {t('support.actions.submitNewTicket')}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        {isLoading ? (
          <div className="text-center py-12">
            <div className={`animate-spin rounded-full h-12 w-12 border-b-2 mx-auto ${resolvedTheme === 'dark' ? 'border-blue-400' : 'border-blue-600'}`}></div>
            <p className={`mt-4 ${resolvedTheme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{t('support.loadingTickets')}</p>
          </div>
        ) : filteredTickets.length === 0 ? (
          <div className="text-center py-12">
            <MessageSquare className={`h-16 w-16 mx-auto mb-4 ${resolvedTheme === 'dark' ? 'text-gray-600' : 'text-gray-400'}`} />
            <h3 className={`text-xl font-medium mb-2 ${resolvedTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              {t('support.empty.title')}
            </h3>
            <p className={`${resolvedTheme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
              {searchTerm || statusFilter !== 'all' || priorityFilter !== 'all' 
                ? t('support.empty.tryAdjust')
                : t('support.empty.noTicketsYet')
              }
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className={`${resolvedTheme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-3 line-clamp-2`}>
                      {ticket.description}
                    </p>
                    <div className="flex items-center space-x-4 text-sm">
